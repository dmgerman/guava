...
parameter_list|()
block|{
comment|/*      * Bug #5692 occurred when TopKSelector called Arrays.sort incorrectly. Test data that would      * trigger a problematic call to Arrays.sort is hard to construct by hand, so we searched for      * one among randomly generated inputs. To reach the Arrays.sort call, we need to pass an input      * that requires many iterations of partitioning inside trim(). So, to construct our random      * inputs, we concatenated 10 sorted lists together.      */
name|int
name|k
init|=
literal|10000
...
name|Random
argument_list|(
literal|1629833645599L
argument_list|)
decl_stmt|;
comment|// target list to be sorted using TopKSelector
name|List
argument_list|<
name|Integer
argument_list|>
