...
begin_comment
comment|/**  * Tests relating to cache computation: concurrent computation, exceptions in computation, etc.  *  * @author schmoe@google.com (mike nonemacher)  */
end_comment
...
argument_list|()
expr_stmt|;
comment|// computation is in progress; asMap shouldn't contain anything
name|ConcurrentMap
argument_list|<
name|String
argument_list|,
...
expr_stmt|;
block|}
DECL|method|testKeyCollectedDuringComputation ()
specifier|public
name|void
name|testKeyCollectedDuringComputation
parameter_list|()
throws|throws
name|InterruptedException
block|{
...
argument_list|()
expr_stmt|;
comment|// Clear key during computation. Note that the computer maintains a
comment|// reference to it, so this should never happen.
name|CacheTesting
operator|.
name|simulateKeyReclamation
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// race condition: the second thread may not have shared the computation (but typically does)
name|assertTrue
argument_list|(
name|callCount
operator|.
