...
expr_stmt|;
block|}
DECL|method|testBase64Url ()
specifier|public
name|void
name|testBase64Url
parameter_list|()
block|{
name|testDecodesByBytes
argument_list|(
name|base64Url
argument_list|()
argument_list|,
...
expr_stmt|;
block|}
DECL|method|testBase64UrlInvalidDecodings ()
specifier|public
name|void
name|testBase64UrlInvalidDecodings
parameter_list|()
block|{
name|assertFailsToDecode
argument_list|(
...
expr_stmt|;
block|}
DECL|method|testDecodesByBytes (BaseEncoding encoding, String encoded, byte[] decoded)
specifier|private
specifier|static
name|void
name|testDecodesByBytes
...
parameter_list|)
block|{
comment|// We use this somewhat weird pattern with an enum for each assertion we want to make as a way
comment|// of dealing with the fact that one of the assertions is @GwtIncompatible but we don't want to
comment|// have to have duplicate @GwtIncompatible test methods just to make that assertion.
for|for
control|(
name|AssertFailsToDecodeStrategy
name|strategy
range|:
name|AssertFailsToDecodeStrategy
operator|.
...
block|}
block|}
DECL|enum|AssertFailsToDecodeStrategy
enum|enum
name|AssertFailsToDecodeStrategy
block|{
DECL|enumConstant|GwtIncompatible
annotation|@
name|GwtIncompatible
comment|// decodingStream(Reader)
DECL|enumConstant|DECODING_STREAM
...
parameter_list|)
block|{
comment|// Regression test for case where DecodingException was swallowed by default implementation
comment|// of
comment|// InputStream.read(byte[], int, int)
comment|// See https://github.com/google/guava/issues/3542
name|Reader
name|reader
init|=
operator|new
...
parameter_list|)
block|{
comment|// Don't assert on the expectedMessage; the messages for exceptions thrown from the
comment|// decoding stream may differ from the messages for the decode methods.
block|}
catch|catch
parameter_list|(
name|IOException
...
name|fail
argument_list|(
literal|"Expected DecodingException but got: "
operator|+
name|e
argument_list|)
expr_stmt|;
...
block|}
block|,
DECL|enumConstant|DECODE_CHECKED
name|DECODE_CHECKED
block|{
annotation|@
name|Override
