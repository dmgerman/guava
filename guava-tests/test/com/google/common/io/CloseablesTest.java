...
begin_comment
comment|/**  * Unit tests for {@link Closeables} and {@link Flushables}.  *  *<p>Checks proper closing and flushing behavior, and ensures that  * IOExceptions on Closeable.close() or Flushable.flush() are not  * propagated out from the {@link Closeables#close} method if {@code  * swallowException} is true.  *  * @author Michael Lancaster  */
end_comment
...
comment|// throw an exception, either on close, on flush, or both.
name|setupCloseable
argument_list|(
literal|true
argument_list|)
...
expr_stmt|;
block|}
comment|// Set up a flushable to expect to be flushed and closed, and optionally to
