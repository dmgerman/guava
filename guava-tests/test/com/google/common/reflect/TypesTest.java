...
decl_stmt|;
comment|// This must be static because the point where it is needed may be earlier than the execution
comment|// of the specific test case that uses it. Currently we need it to operate in the static
comment|// initializer of TypeVariableInvocationHandler.
DECL|field|securityManager
specifier|private
specifier|static
specifier|final
name|NoSetAccessibleSecurityManager
name|securityManager
init|=
operator|new
name|NoSetAccessibleSecurityManager
argument_list|()
decl_stmt|;
comment|/**    * A SecurityManager that disallows {@link java.lang.reflect.Method#setAccessible(boolean)}    * when it comes from a specific place. The idea is to ensure that    * {@code Types.newTypeVariableImpl}, which calls {@code setAccessible}, can still work if that    * call is refused. We also record whether the refusal happened, so that we can test that we are    * indeed refusing access where we expect.    */
DECL|class|NoSetAccessibleSecurityManager
specifier|private
specifier|static
class|class
name|NoSetAccessibleSecurityManager
...
name|SecurityManager
block|{
DECL|field|DISALLOWED_PERMISSION
specifier|private
specifier|static
specifier|final
name|Permission
name|DISALLOWED_PERMISSION
init|=
operator|new
name|ReflectPermission
argument_list|(
...
argument_list|)
decl_stmt|;
DECL|field|setAccessibleWasRefused
specifier|volatile
name|boolean
name|setAccessibleWasRefused
decl_stmt|;
annotation|@
name|Override
DECL|method|checkPermission (Permission p)
specifier|public
name|void
...
argument_list|)
operator|&&
name|callStackContainsMethod
argument_list|(
literal|"TypeVariableInvocationHandler"
argument_list|,
literal|"<clinit>"
argument_list|)
condition|)
...
name|fail
argument_list|(
literal|"Did not get expected AccessControlException"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
block|}
block|}
DECL|method|callStackContainsMethod (String classNameFragment, String methodName)
specifier|private
specifier|static
name|boolean
name|callStackContainsMethod
...
argument_list|()
expr_stmt|;
comment|// At some point before now we should have hit the permission check that prevents us from
comment|// calling setAccessible. We check that that is indeed true because we want to make sure
comment|// that creating a TypeVariable instance works even when that is not allowed.
name|assertTrue
argument_list|(
name|securityManager
operator|.
...
return|;
block|}
DECL|class|TypeVariableEqualsTester
specifier|private
specifier|static
class|class
name|TypeVariableEqualsTester
...
argument_list|()
decl_stmt|;
DECL|method|addEqualityGroup (Type jvmType, Type... types)
name|TypeVariableEqualsTester
name|addEqualityGroup
parameter_list|(
name|Type
