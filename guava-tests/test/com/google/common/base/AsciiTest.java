...
argument_list|)
expr_stmt|;
comment|// Create new strings here to avoid early-out logic.
name|assertTrue
argument_list|(
name|Ascii
operator|.
...
argument_list|)
expr_stmt|;
comment|// Compare to: "\u00c1".equalsIgnoreCase("\u00e1") == true
name|assertFalse
argument_list|(
name|Ascii
operator|.
...
argument_list|)
expr_stmt|;
comment|// Test chars just outside the alphabetic range ('A'-1 vs 'a'-1, 'Z'+1 vs 'z'+1)
name|assertFalse
argument_list|(
name|Ascii
operator|.
...
specifier|public
name|void
name|testEqualsIgnoreCaseUnicodeEquivalence
parameter_list|()
block|{
comment|// Note that it's possible in future that the JDK's idea to toUpperCase() or equalsIgnoreCase()
comment|// may change and break assumptions in this test [*]. This is not a bug in the implementation of
comment|// Ascii.equalsIgnoreCase(), but it is a signal that its documentation may need updating as
comment|// regards edge cases.
