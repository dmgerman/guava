...
expr_stmt|;
block|}
comment|/*    * Tests for Predicates.instanceOf(x).    */
...
expr_stmt|;
block|}
comment|/*    * Tests for Predicates.contains(Pattern) and .containsPattern(String).    * We assume the regex level works, so there are only trivial tests of that    * aspect.    */
...
expr_stmt|;
block|}
DECL|method|checkConsistency ( Predicate<? super Integer> expected, Predicate<? super Integer> actual)
specifier|public
name|void
...
expr_stmt|;
block|}
DECL|method|testHashCodeForBooleanOperationsIsConsistentWithBooleanLogic ()
specifier|public
name|void
name|testHashCodeForBooleanOperationsIsConsistentWithBooleanLogic
parameter_list|()
block|{
comment|/*      * This isn't a "requirement" yet, as much as it's a "nice to have for      * future design."      *      * Maybe it will be possible to eventually have these predicates do      * certain simplifying logical transformations to simpler equivalent      * forms.  If so, this checks that the hash codes have been chosen in such      * a way that the fundamental building-block operations:      *      *    alwaysTrue()      *    alwaysFalse()      *    not(p)      *    and(p1, p2)      *    or(p1, p2)      *      * have hashCode() calculations that coincidentally cause equivalent logical      * expressions to have equivalent hashCode() values.      */
name|Predicate
argument_list|<
name|Integer
argument_list|>
...
argument_list|)
expr_stmt|;
comment|/*      * Now that alwaysFalse and alwaysTrue follow the enum singleton pattern,      * the following tests can't call checkConsistency to compare the hash      * codes.      */
name|assertEvalsLike
argument_list|(
name|Predicates
operator|.
...
expr_stmt|;
block|}
DECL|method|assertEvalsLike ( Predicate<? super Integer> expected, Predicate<? super Integer> actual, Integer input)
specifier|private
specifier|static
name|void
name|assertEvalsLike
