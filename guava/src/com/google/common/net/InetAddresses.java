...
literal|':'
decl_stmt|;
DECL|field|IPV4_DELIMITER_MATCHER
specifier|private
specifier|static
specifier|final
name|CharMatcher
name|IPV4_DELIMITER_MATCHER
init|=
name|CharMatcher
operator|.
name|is
...
argument_list|)
decl_stmt|;
DECL|field|IPV6_DELIMITER_MATCHER
specifier|private
specifier|static
specifier|final
name|CharMatcher
name|IPV6_DELIMITER_MATCHER
init|=
name|CharMatcher
operator|.
name|is
...
argument_list|)
expr_stmt|;
comment|// Given a 4-byte array, this cast should always succeed.
return|return
operator|(
name|Inet4Address
operator|)
...
literal|null
return|;
comment|// Scope IDs are not supported for IPV4
block|}
return|return
...
literal|0
decl_stmt|;
comment|// Iterate through the parts of the ip string.
comment|// Invariant: start is always the beginning of an octet.
for|for
control|(
name|int
name|i
...
operator|)
decl_stmt|;
comment|// estimate; may be modified later
name|boolean
name|hasSkip
init|=
literal|false
decl_stmt|;
comment|// Scan for the appearance of ::, to mark a skip-format IPV6 string and adjust the partsSkipped
comment|// estimate.
for|for
control|(
name|int
...
operator|++
expr_stmt|;
comment|// :: means we skipped an extra part in between the two delimiters.
if|if
condition|(
name|i
operator|==
...
operator|++
expr_stmt|;
comment|// Begins with ::, so we skipped the part preceding the first :
block|}
if|if
condition|(
name|i
...
operator|++
expr_stmt|;
comment|// Ends with ::, so we skipped the part after the last :
block|}
block|}
block|}
if|if
...
literal|null
return|;
comment|// :: must expand to at least one '0'
block|}
if|if
condition|(
operator|!
...
literal|null
return|;
comment|// Incorrect number of parts
block|}
name|ByteBuffer
name|rawBytes
init|=
...
block|{
comment|// Iterate through the parts of the ip string.
comment|// Invariant: start is always the beginning of a hextet, or the second ':' of the skip
comment|// sequence "::"
name|int
name|start
init|=
...
return|;
block|}
DECL|method|parseOctet (String ipString, int start, int end)
specifier|private
specifier|static
name|byte
name|parseOctet
...
parameter_list|)
block|{
comment|// Note: we already verified that this string contains only hex digits, but the string may still
comment|// contain non-decimal characters.
name|int
name|length
init|=
name|end
...
return|;
block|}
comment|// Parse a hextet out of the ipString from start (inclusive) to end (exclusive)
DECL|method|parseHextet (String ipString, int start, int end)
specifier|private
specifier|static
name|short
name|parseHextet
...
parameter_list|)
block|{
comment|// While scanning the array, handle these state transitions:
comment|//   start->num => "num"     start->gap => "::"
comment|//   num->num   => ":num"    num->gap   => "::"
comment|//   gap->num   => "num"     gap->gap   => ""
name|StringBuilder
name|buf
init|=
operator|new
...
name|formatIllegalArgumentException
argument_list|(
literal|"Not a valid URI IP literal: '%s'"
argument_list|,
name|hostAddr
argument_list|)
throw|;
...
argument_list|)
expr_stmt|;
comment|// Decide if this should be an IPv6 or IPv4 address.
name|String
name|ipString
decl_stmt|;
name|int
...
expr_stmt|;
block|}
comment|// Parse the address, and make sure the length/version is correct.
name|byte
index|[]
name|addr
init|=
...
name|copyLength
decl_stmt|;
comment|// Check the extra bytes in the BigInteger are all zero.
for|for
control|(
name|int
name|i
...
return|;
block|}
comment|/**    * Returns a new InetAddress that is one less than the passed in address. This method works for    * both IPv4 and IPv6 addresses.    *    * @param address the InetAddress to decrement    * @return a new InetAddress that is one less than the passed in address    * @throws IllegalArgumentException if InetAddress is at the beginning of its range    * @since 18.0    */
DECL|method|decrement (InetAddress address)
specifier|public
specifier|static
name|InetAddress
name|decrement
...
literal|0
argument_list|,
literal|"Decrementing %s would wrap."
argument_list|,
name|address
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|formatIllegalArgumentException ( String format, Object... args)
specifier|private
specifier|static
