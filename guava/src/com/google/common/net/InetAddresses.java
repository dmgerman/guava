...
condition|)
block|{
comment|// For the special mapped address cases (e.g. "::ffff:192.0.2.1") passing
comment|// InetAddress.getByAddress() the output of textToNumericFormatV6()
comment|// below will "do the right thing", i.e. construct an Inet4Address.
return|return
literal|null
return|;
block|}
...
argument_list|)
decl_stmt|;
comment|// Keep a record of the number of parts specified above/before a "::"
comment|// (partsHi), and below/after any "::" (partsLo).
name|int
name|partsHi
init|=
literal|0
...
decl_stmt|;
comment|// At most 1 "::".
comment|// Parse parts above any "::", or the whole string if no "::" present.
if|if
condition|(
operator|!
name|addressHalves
...
condition|)
block|{
comment|// No empty segments permitted.
return|return
literal|null
return|;
block|}
...
block|{
comment|// A leading "::".  At least one 16bit segment must be zero.
name|partsHi
operator|=
literal|1
expr_stmt|;
block|}
comment|// Parse parts below "::" (if any), into the tail end of the byte array,
...
block|{
comment|// A trailing "::".  At least one 16bit segment must be zero.
name|partsLo
operator|=
literal|1
expr_stmt|;
block|}
block|}
comment|// Some extra sanity checks.
name|int
name|totalParts
init|=
name|partsHi
...
condition|)
block|{
comment|// If no "::" shortening is used then all bytes must have been specified.
return|return
literal|null
return|;
block|}
