...
name|x
operator|+
literal|") is not representable as a long"
argument_list|)
throw|;
block|}
return|return
...
argument_list|)
expr_stmt|;
comment|// Long.highestOneBit was buggy on GWT.  We've fixed it, but I'm not certain when the fix will
comment|// be released.
return|return
literal|1L
operator|<<
...
condition|)
block|{
comment|// If a and b have different signs or a has the same sign as the result then there was no
comment|// overflow, return.
return|return
name|naiveSum
return|;
block|}
comment|// we did over/under flow, if the sign is negative we should return MAX otherwise MIN
return|return
name|Long
operator|.
name|MAX_VALUE
...
condition|)
block|{
comment|// If a and b have the same signs or a has the same sign as the result then there was no
comment|// overflow, return.
return|return
name|naiveDifference
return|;
block|}
comment|// we did over/under flow
return|return
name|Long
operator|.
name|MAX_VALUE
...
parameter_list|)
block|{
comment|// see checkedMultiply for explanation
name|int
name|leadingZeros
init|=
name|Long
...
return|;
block|}
comment|// the return value if we will overflow (which we calculate by overflowing a long :) )
name|long
name|limit
init|=
name|Long
...
return|;
block|}
comment|/*    * This bitmask is used as an optimization for cheaply testing for divisiblity by 2, 3, or 5.    * Each bit is set to 1 for all remainders that indicate divisibility by 2, 3, or 5, so    * 1, 7, 11, 13, 17, 19, 23, 29 are set to 0. 30 and up don't matter because they won't be hit.    */
DECL|field|SIEVE_30
specifier|private
specifier|static
specifier|final
...
condition|)
block|{
comment|// Encode all primes less than 66 into mask without 0 and 1.
name|long
name|mask
init|=
operator|(
...
operator|)
decl_stmt|;
comment|// Look up n within the mask.
return|return
operator|(
operator|(
name|mask
...
parameter_list|)
block|{
comment|/*          * lowasser, 2015-Feb-12: Benchmarks suggest that changing this to UnsignedLongs.remainder          * and increasing the threshold to 2^32 doesn't pay for itself, and adding another enum          * constant hurts performance further -- I suspect because bimorphic implementation is a          * sweet spot for the JVM.          */
return|return
operator|(
name|a
operator|*
...
annotation|@
name|GwtIncompatible
DECL|method|roundToDouble (long x, RoundingMode mode)
specifier|public
specifier|static
name|double
name|roundToDouble
