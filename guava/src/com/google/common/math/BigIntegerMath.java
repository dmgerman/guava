...
block|{
comment|// Too big for double precision!  Use a less accurate initial approximation,
comment|// specifically 2^(log2(x) / 2).
name|int
name|logFloor
init|=
name|log2
...
argument_list|)
expr_stmt|;
comment|// In exchange, we can use a right-shift instead of a divide.
name|sqrt1
operator|=
name|sqrt0
operator|.
