...
block|{
comment|// Updating algorithm (1.5) from Chan et al. (1983). Algorithms for Computing the Sample
comment|// Variance: Analysis and Recommendations. The American Statistician 37, 242-247.
name|long
name|nextCount
init|=
name|count
...
name|mean
decl_stmt|;
comment|// Note that this is the naive mean formula, so non-finite values are handled naturally.
comment|// TODO(b/26080783): Decide whether to use naive or Knuth formula. Consider MAX_VALUE case.
name|mean
operator|=
operator|(
name|sum
...
name|nextCount
expr_stmt|;
comment|// Note that non-finite inputs will have sumOfSquaresOfDeltas = NaN, so non-finite values will
comment|// result in NaN naturally.
name|sumOfSquaresOfDeltas
operator|+=
name|values
operator|.
