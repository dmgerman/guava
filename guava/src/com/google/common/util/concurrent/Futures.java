...
name|outputFuture
decl_stmt|;
DECL|method|WrappedCombiner (Callable<T> delegate)
name|WrappedCombiner
parameter_list|(
name|Callable
argument_list|<
...
expr_stmt|;
block|}
comment|// at this point the return value doesn't matter since we already called setException or
comment|// cancel so the future is done.
return|return
literal|null
return|;
block|}
block|}
DECL|class|CombinerFuture
specifier|private
specifier|static
specifier|final
class|class
...
name|futures
decl_stmt|;
DECL|method|CombinerFuture (Callable<V> callable, ImmutableList<ListenableFuture<?>> futures)
name|CombinerFuture
parameter_list|(
name|Callable
argument_list|<
...
comment|// have sameThreadExecutor listeners. In this situation, the listeners may end up running on a
...
comment|// 1. Using the sameThreadExecutor implies that your callback is safe to run on any thread.
comment|// 2. This would likely only be noticeable if you were doing something expensive or blocking on
comment|//    a sameThreadExecutor listener on one of the output futures which is an antipattern anyway.
