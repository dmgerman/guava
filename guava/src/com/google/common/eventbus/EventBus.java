...
name|EventBus
block|{
comment|/**    * A thread-safe cache for flattenHierarchy(). The Class class is immutable. This cache is shared    * across all EventBus instances, which greatly improves performance if multiple such instances    * are created and objects of the same class are posted on all of them.    */
...
begin_decl_stmt
DECL|field|subscriberExceptionHandler
specifier|private
...
begin_comment
comment|/**    * Creates a new EventBus with the given {@link SubscriberExceptionHandler}.    *     * @param subscriberExceptionHandler Handler for subscriber exceptions.    * @since 16.0    */
end_comment

begin_constructor
DECL|method|EventBus (SubscriberExceptionHandler subscriberExceptionHandler)
specifier|public
name|EventBus
parameter_list|(
name|SubscriberExceptionHandler
...
parameter_list|)
block|{
comment|// If the exception handler throws, log it. There isn't much else to do!
name|Logger
operator|.
name|getLogger
argument_list|(
...
begin_class
DECL|class|LoggingSubscriberExceptionHandler
specifier|private
specifier|static
specifier|final
class|class
...
name|logger
decl_stmt|;
comment|/**      * @param identifier a brief name for this bus, for logging purposes. Should      *        be a valid Java identifier.      */
DECL|method|LoggingSubscriberExceptionHandler (String identifier)
specifier|public
name|LoggingSubscriberExceptionHandler
parameter_list|(
name|String
