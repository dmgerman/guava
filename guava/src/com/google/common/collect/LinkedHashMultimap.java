...
block|;
DECL|method|ValueEntry ( @arametricNullness K key, @ParametricNullness V value, int smearedValueHash, @CheckForNull ValueEntry<K, V> nextInValueBucket)
name|ValueEntry
argument_list|(
annotation|@
...
literal|"nullness"
argument_list|)
comment|// see the comment on the class fields, especially about newHeader
DECL|method|newHeader ()
specifier|static
operator|<
name|K
expr|extends @
...
return|;
block|}
DECL|method|matchesValue (@heckForNull Object v, int smearedVHash)
name|boolean
name|matchesValue
argument_list|(
annotation|@
...
argument_list|)
return|;
comment|// see the comment on the class fields
block|}
end_function
...
begin_comment
comment|/**    * Returns a view collection of all<i>distinct</i> keys contained in this multimap. Note that the    * key set contains a key if and only if this multimap maps that key to at least one value.    *    *<p>The iterator generated by the returned set traverses the keys in the order they were first    * added to the multimap.    *    *<p>Changes to the returned set will update the underlying multimap, and vice versa. However,    *<i>adding</i> to the returned set is not possible.    */
end_comment
...
name|lastEntry
decl_stmt|;
DECL|method|ValueSet (@arametricNullness K key, int expectedValues)
name|ValueSet
parameter_list|(
annotation|@
name|ParametricNullness
...
argument_list|()
decl_stmt|;
comment|/*        * requireNonNull is safe for a properly serialized multimap: We've already inserted a        * collection for each key that we expect.        */
name|requireNonNull
argument_list|(
name|map
operator|.
