...
return|;
block|}
DECL|method|closeAll (BaseStream<?, ?>[] toClose)
specifier|private
specifier|static
name|void
name|closeAll
...
name|b
block|;
DECL|method|TemporaryPair (@arametricNullness A a, @ParametricNullness B b)
name|TemporaryPair
argument_list|(
annotation|@
name|ParametricNullness
...
block|@
name|ParametricNullness
DECL|method|apply (@arametricNullness T from, long index)
name|R
name|apply
argument_list|(
annotation|@
...
comment|/*    * By declaring<T> instead of<T extends @Nullable Object>, we declare this method as requiring a    * stream whose elements are non-null. However, the method goes out of its way to still handle    * nulls in the stream. This means that the method can safely be used with a stream that contains    * nulls as long as the *last* element is *not* null.    *    * (To "go out of its way," the method tracks a `set` bit so that it can distinguish "the final    * split has a last element of null, so throw NPE" from "the final split was empty, so look for an    * element in the prior one.")    */
expr|@
name|Beta
...
argument_list|()
block|{
comment|/*          * requireNonNull is safe because we call get() only if we've previously called set().          *          * (For further discussion of nullness, see the comment above the method.)          */
return|return
name|requireNonNull
argument_list|(
name|value
