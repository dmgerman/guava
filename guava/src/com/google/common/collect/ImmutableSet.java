...
argument_list|>
block|{
comment|/*      * `impl` is null only for instances of the subclass, ImmutableSortedSet.Builder. That subclass      * overrides all the methods that access it here. Thus, all the methods here can safely assume      * that this field is non-null.      */
DECL|field|impl
annotation|@
name|CheckForNull
specifier|private
...
parameter_list|>
block|{
comment|// The first `distinct` elements are non-null.
comment|// Since we can never access null elements, we don't mark this nullable.
...
control|)
block|{
comment|/*          * requireNonNull is safe because we ensure that the first `distinct` elements have been          * populated.          */
name|result
operator|=
name|result
operator|.
...
function_decl|;
block|}
DECL|class|EmptySetBuilderImpl
specifier|private
specifier|static
specifier|final
class|class
...
return|;
block|}
DECL|method|EmptySetBuilderImpl ()
specifier|private
name|EmptySetBuilderImpl
parameter_list|()
block|{
...
comment|// TODO(cpovirk): Move to Hashing or something, since it's used elsewhere in the Android version.
...
argument_list|>
block|{
comment|// null until at least two elements are present
...
return|;
block|}
DECL|method|insertInHashTable (E e)
specifier|private
name|SetBuilderImpl
argument_list|<
name|E
...
return|;
default|default:
comment|/*            * The suppression is safe because we ensure that the first `distinct` elements have been            * populated.            */
annotation|@
name|SuppressWarnings
argument_list|(
literal|"nullness"
...
control|)
block|{
comment|// requireNonNull is safe because we ensure that the first n elements have been populated.
name|Object
name|e
init|=
name|requireNonNull
...
expr_stmt|;
block|}
comment|/**      * We attempt to detect deliberate hash flooding attempts. If one is detected, we fall back to a      * wrapper around j.u.HashSet, which has built in flooding protection. MAX_RUN_MULTIPLIER was      * determined experimentally to match our desired probability of false positives.      */
comment|// NB: yes, this is surprisingly high, but that's what the experiments said was necessary
comment|// Raising this number slows the worst-case contains behavior, speeds up hashFloodingDetected,
comment|// and reduces the false-positive probability.
...
DECL|method|hashFloodingDetected (@ullable Object[] hashTable)
specifier|static
name|boolean
name|hashFloodingDetected
parameter_list|(
...
literal|1
decl_stmt|;
comment|// Invariant: all elements at indices in [knownRunStart, knownRunEnd) are nonnull.
comment|// If knownRunStart == knownRunEnd, this is vacuously true.
comment|// When knownRunEnd exceeds hashTable.length, it "wraps", detecting runs around the end
...
condition|)
block|{
comment|// There are only maxRunBeforeFallback - 1 elements between here and there,
comment|// so even if they were all nonnull, we wouldn't detect a hash flood.  Therefore,
comment|// we can skip them all.
name|knownRunStart
operator|+=
name|maxRunBeforeFallback
expr_stmt|;
...
operator|++
expr_stmt|;
comment|// the only case in which maxRunEnd doesn't increase by mRBF
comment|// happens about f * (1-f) for f = DESIRED_LOAD_FACTOR, so around 21% of the time
block|}
name|knownRunEnd
operator|=
name|knownRunStart
