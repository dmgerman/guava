...
condition|)
block|{
comment|// probable hash flooding attack, fall back to j.u.HM based implementation and use its
comment|// implementation of hash flooding protection
return|return
...
return|;
block|}
comment|/** Makes an entry usable internally by a new ImmutableMap without rereading its contents. */
DECL|method|makeImmutable (Entry<K, V> entry, K key, V value)
specifier|static
parameter_list|<
name|K
parameter_list|,
...
return|;
block|}
comment|/** Makes an entry usable internally by a new ImmutableMap. */
DECL|method|makeImmutable (Entry<K, V> entry)
specifier|static
parameter_list|<
name|K
parameter_list|,
...
name|map
decl_stmt|;
DECL|method|KeySet (RegularImmutableMap<K, ?> map)
name|KeySet
parameter_list|(
name|RegularImmutableMap
argument_list|<
...
return|;
block|}
comment|// No longer used for new writes, but kept so that old data can still be read.
annotation|@
name|GwtIncompatible
