...
parameter_list|)
block|{
comment|// TODO(user): when we link values instead of entries this method can go
...
block|}
comment|// eviction
DECL|method|enqueueNotification (K key, int hash, ValueReference<K, V> valueReference)
name|void
...
condition|)
block|{
comment|// we may be expiring a large number of entries, so enqueue future cleanup rather than
comment|// removing them immediately
if|if
condition|(
operator|!
name|removeEntry
...
block|{
comment|// immediately reuse partially collected entry, but send notifications first
name|clearLiveEntry
argument_list|(
name|e
argument_list|,
...
block|}
block|}
comment|// entry removal and cleanup
comment|/**      * Removes an entry from within a table. All entries following the removed node can stay, but      * all preceding ones need to be cloned. Count is not decremented, as the entry could have      * already been removed from the count; however partially collected entries are cleared, which      * could indirectly result in count being decremented. Thus callers of this method which are      * removing live entries need to both re-read count and decrement it.      *      * @param first the first entry of the table      * @param entry the entry being removed from the table      * @return the new first entry for the table      */
annotation|@
name|GuardedBy
argument_list|(
literal|"Segment.this"
...
argument_list|)
expr_stmt|;
comment|// entry not yet added to eviction/expiration queues
return|return
literal|true
return|;
block|}
...
block|}
block|}
comment|/**      * Immediately removes an entry if it is still live.      */
annotation|@
name|GuardedBy
argument_list|(
literal|"Segment.this"
...
return|;
block|}
comment|/**      * Immediately removes a live entry from a table. This updates the table, thus invalidating any      * partial iteration over its entries by the caller.      */
annotation|@
name|GuardedBy
argument_list|(
literal|"Segment.this"
argument_list|)
DECL|method|removeLiveEntry (AtomicReferenceArray<ReferenceEntry<K, V>> table, int index, ReferenceEntry<K, V> first, ReferenceEntry<K, V> entry, int hash, ValueReference<K, V> valueReference)
name|boolean
name|removeLiveEntry
parameter_list|(
name|AtomicReferenceArray
...
return|;
block|}
comment|/**      * Clears an entry which is being removed by sending removal notification and removing it from      * all queues. This method decrements count, and thus should be called after {@code table.set}      * in order for the volatile write of count to force visibility of the updated table.      */
annotation|@
name|GuardedBy
argument_list|(
literal|"Segment.this"
argument_list|)
DECL|method|clearLiveEntry (ReferenceEntry<K, V> entry, int hash, ValueReference<K, V> valueReference)
name|void
name|clearLiveEntry
parameter_list|(
name|ReferenceEntry
...
argument_list|()
decl_stmt|;
comment|// TODO(user): call getLiveValue when it's moved out of Segment
name|V
name|value
init|=
name|entry
