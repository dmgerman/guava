...
begin_comment
comment|/**  * Provides static methods acting on or generating a {@code Multimap}.  *  * @author Jared Levy  * @author Robert Konigsberg  * @author Mike Bostock  * @author Louis Wasserman  * @since 2.0 (imported from Google Collections Library)  */
end_comment
...
argument_list|>
block|{
DECL|method|TransformedEntries ( final EntryTransformer<? super K, ? super V1, V2> transformer)
name|TransformedEntries
parameter_list|(
specifier|final
name|EntryTransformer
...
return|;
block|}
comment|/**    *<b>Deprecated.</b>    *    * @since 10.0    * @deprecated use {@link #index(Iterator, Function)} by casting {@code    *     values} to {@code Iterator<V>}, or better yet, by implementing only    *     {@code Iterator} and not {@code Iterable}.<b>This method is scheduled    *     for deletion in March 2012.</b>    */
annotation|@
name|Beta
annotation|@
name|Deprecated
...
argument_list|>
parameter_list|>
DECL|method|index ( I values, Function<? super V, K> keyFunction)
name|ImmutableListMultimap
argument_list|<
name|K
argument_list|,
...
name|predicate
decl_stmt|;
DECL|method|FilteredMultimap (Multimap<K, V> unfiltered, Predicate<? super Entry<K, V>> predicate)
name|FilteredMultimap
parameter_list|(
name|Multimap
argument_list|<
...
return|;
block|}
comment|// This method should be called only when key is a K and value is a V.
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
argument_list|)
DECL|method|satisfiesPredicate (Object key, Object value)
name|boolean
name|satisfiesPredicate
parameter_list|(
name|Object
name|key
parameter_list|,
...
expr_stmt|;
block|}
comment|// Not calling unfiltered.replaceValues() since values that don't satisify
comment|// the filter should remain in the multimap.
name|Collection
argument_list|<
name|V
argument_list|>
...
block|}
block|}
DECL|method|filterCollection (Collection<V> collection, Predicate<V> predicate)
name|Collection
argument_list|<
name|V
argument_list|>
...
return|;
block|}
comment|// Override remove methods since iterator doesn't support remove.
DECL|method|remove (Object o)
annotation|@
name|Override
specifier|public
...
return|;
block|}
comment|/**      * Remove all filtered asMap() entries that satisfy the predicate.      */
DECL|method|removeEntriesIf (Predicate<Map.Entry<K, Collection<V>>> removalPredicate)
name|boolean
...
condition|)
block|{
comment|// Determine whether to remove the filtered values with this key.
name|Map
operator|.
name|Entry
argument_list|<
...
argument_list|()
expr_stmt|;
comment|// Remove all values for the key.
block|}
else|else
block|{
name|filteredCollection
...
argument_list|()
expr_stmt|;
comment|// Remove the filtered values only.
block|}
block|}
block|}
return|return
...
parameter_list|()
block|{
comment|// Select the values that satisify the predicate.
name|EntryTransformer
argument_list|<
name|K
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// Select the keys that have at least one value remaining.
name|Map
argument_list|<
name|K
argument_list|,
...
argument_list|)
decl_stmt|;
comment|// Override the removal methods, since removing a map entry should not
comment|// affect values that don't satisfy the filter.
return|return
operator|new
name|AsMap
argument_list|(
...
name|delegate
decl_stmt|;
DECL|method|AsMap (Map<K, Collection<V>> delegate)
name|AsMap
parameter_list|(
name|Map
argument_list|<
...
argument_list|>
argument_list|>
name|delegateEntries
decl_stmt|;
DECL|method|EntrySet (Set<Map.Entry<K, Collection<V>>> delegateEntries)
specifier|public
name|EntrySet
parameter_list|(
name|Set
...
return|;
block|}
DECL|method|remove (Object o, int occurrences)
annotation|@
name|Override
specifier|public
name|int
