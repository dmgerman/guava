...
parameter_list|)
function_decl|;
comment|/**    * Returns the hash code for this multimap.    *    *<p>The hash code of a multimap is defined as the hash code of the map view,    * as returned by {@link Multimap#asMap}.    *    *<p><b>Warning:</b> because {@link Collection#hashCode} is generally    * unspecified, multimaps containing the same key-value pairs, even in the    * same order, may or may not have the same hash code. The recommended    * subinterfaces provide a much stronger guarantee.    */
annotation|@
name|Override
