...
return|;
block|}
comment|/**    * Specifies that each key (not value) stored in the map should be wrapped in a    * {@link SoftReference} (by default, strong references are used). Softly-referenced objects will    * be garbage-collected in a<i>globally</i> least-recently-used manner, in response to memory    * demand.    *    *<p><b>Warning:</b> in most circumstances it is better to set a per-cache {@linkplain    * #maximumSize maximum size} instead of using soft references. You should only use this method if    * you are well familiar with the practical consequences of soft references.    *    *<p><b>Warning:</b> when this method is used, the resulting map will use identity ({@code ==})    * comparison to determine equality of keys, which is a technical violation of the {@link Map}    * specification, and may not be what you expect.    *    * @throws IllegalStateException if the key strength was already set    * @see SoftReference    */
annotation|@
name|GwtIncompatible
argument_list|(
...
return|;
block|}
comment|/**    * Builds a cache, which either returns an already-loaded value for a given key or atomically    * computes or retrieves it using the supplied {@code CacheLoader}. If another thread is currently    * loading the value for this key, simply waits for that thread to finish and returns its    * loaded value. Note that multiple threads can concurrently load values for distinct keys.    *    *<p>The returned {@link Cache} implements the optional operations {@link Cache#invalidate},    * {@link Cache#invalidateAll}, {@link Cache#size}, {@link Cache#stats}, and {@link Cache#asMap},    * with the following qualifications:    *    *<ul>    *<li>The {@code invalidateAll} method will invalidate all cached entries prior to returning, and    *     removal notifications will be issued for all invalidated entries.    *<li>The {@code asMap} view supports removal operations, but no other modifications.    *</ul>    *    *<p>This method does not alter the state of this {@code MapMaker} instance, so it can be invoked    * again to create multiple independent caches.    *    * @param loader the cache loader used to obtain new values    * @return a cache having the requested features    * @since Guava release 10    */
annotation|@
name|Beta
annotation|@
name|Override
...
name|loader
decl_stmt|;
DECL|method|NullComputingConcurrentMap (MapMaker mapMaker, CacheLoader<? super K, ? extends V> loader)
name|NullComputingConcurrentMap
parameter_list|(
name|MapMaker
name|mapMaker
...
name|statsCounter
decl_stmt|;
DECL|method|NullCache (MapMaker mapMaker, Supplier<? extends StatsCounter> statsCounterSupplier, CacheLoader<? super K, V> loader)
name|NullCache
parameter_list|(
name|MapMaker
name|mapMaker
