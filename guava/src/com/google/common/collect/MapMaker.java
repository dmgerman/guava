...
return|;
block|}
comment|/**    * Specifies the maximum number of entries the map may contain. While the number of entries in the    * map is not guaranteed to grow to the maximum, the map will attempt to make the best use of    * memory without exceeding the maximum number of entries. As the map size grows close to the    * maximum, the map will evict entries that are less likely to be used again. For example, the map    * may evict an entry because it hasn't been used recently or very often.    *    *<p>When {@code size} is zero, elements can be successfully added to the map, but are evicted    * immediately.    *    * @param size the maximum size of the map    *    * @throws IllegalArgumentException if {@code size} is negative    * @throws IllegalStateException if a maximum size was already set    * @since Guava release 08    */
annotation|@
name|Beta
annotation|@
name|Override
