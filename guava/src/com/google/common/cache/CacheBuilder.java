...
return|;
block|}
comment|/**    * Specifies that active entries are eligible for automatic refresh once a fixed duration has    * elapsed after the entry's creation, or the most recent replacement of its value. The semantics    * of refreshes are specified in {@link LoadingCache#refresh}, and are performed by calling    * {@link CacheLoader.reload}.    *    *<p>Refreshes will only be asynchronous if the implementation of {@link CacheLoader.reload} is    * asynchronous.    *    *<p>Currently automatic refreshes are performed when the first stale request for an entry    * occurs. If the refresh completes immediately then the new value will be returned; otherwise the    * old value will continue to be returned until the refresh completes.    *    * @param duration the length of time after an entry is created that it should be considered    *     stale, and thus eligible for refresh    * @param unit the unit that {@code duration} is expressed in    * @throws IllegalArgumentException if {@code duration} is negative    * @throws IllegalStateException if the refresh interval was already set    * @since 11.0    */
