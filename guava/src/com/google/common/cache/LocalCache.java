...
argument_list|)
decl_stmt|;
comment|// update loadingFuture for the sake of other pending requests
name|loadingValueReference
operator|.
name|set
argument_list|(
...
return|;
block|}
DECL|method|setException (SettableFuture<?> future, Throwable t)
specifier|private
specifier|static
name|boolean
name|setException
...
parameter_list|)
block|{
comment|// the error will already be propagated by the loading thread
return|return
literal|false
return|;
block|}
...
argument_list|)
decl_stmt|;
comment|// rely on loadAsync to call set in order to avoid adding a second listener here
return|return
name|newValue
operator|!=
literal|null
...
expr_stmt|;
block|}
comment|/*      * These methods have been moved to LoadingCache, but they temporarily      * remain in Cache in Guava.      */
DECL|method|get (K key)
specifier|public
name|V
name|get
