...
block|}
block|}
comment|/**    * Returns the lexically cleaned form of the path name:    *     *<ul>    *<li>empty string becomes .    *<li>fold out ../ when possible    *<li>fold out ./ when possible    *<li>collapse multiple slashes    *</li>delete trailing slashes (unless the path is just "/")    *</ul>    *    * @since Guava release 10    */
...
argument_list|()
decl_stmt|;
comment|// In place, rewrite name to compress multiple /, eliminate ., and process ..
name|boolean
name|rooted
init|=
operator|(
...
decl_stmt|;
comment|// invariants:
comment|//  p points at beginning of path element we're considering.
comment|//  q points just past the last path element we wrote (no slash).
comment|//  dotdot points just past the point where .. cannot backtrack
comment|//    any further (no slash).
name|int
name|firstNonSlash
init|=
name|rooted
...
condition|)
block|{
comment|/* don't count the separator in case it is null */
name|p
operator|+=
literal|1
expr_stmt|;
...
return|;
block|}
DECL|method|sep (char[] a, int pos)
specifier|private
specifier|static
name|boolean
name|sep
