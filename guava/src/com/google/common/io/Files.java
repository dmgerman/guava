...
argument_list|()
decl_stmt|;
comment|// some special files may return size 0 but have content
comment|// read normally to be sure
if|if
condition|(
name|size
operator|==
...
return|;
block|}
comment|// can't initialize a large enough array
comment|// technically, this could probably be Integer.MAX_VALUE - 5
if|if
condition|(
name|size
operator|>
...
condition|)
block|{
comment|// OOME is what would be thrown if we tried to initialize the array
throw|throw
operator|new
...
throw|;
block|}
comment|// initialize the array to the current size of the file
name|byte
index|[]
name|bytes
init|=
...
literal|0
decl_stmt|;
comment|// read until we've read size bytes or reached EOF
while|while
condition|(
name|off
operator|<
