...
name|ByteStreams
block|{
comment|/**    * Default size of buffers allocated for copies.    */
DECL|field|BUF_SIZE
specifier|static
specifier|final
name|int
...
literal|8192
decl_stmt|;
comment|/**    * A buffer for skipping bytes in an input stream. Only written to and never read, so actual    * contents don't matter.    */
...
block|{
comment|// Since skipped is likely considerably smaller than skipBuffer's length and this should
comment|// likely only rarely occur, this is probably better than filling the whole buffer in a
comment|// finally block even though it could theoretically happen multiple times in some cases.
name|Arrays
operator|.
name|fill
argument_list|(
...
parameter_list|)
block|{
comment|// If an exception occurs, skipBuffer could have been partially written to and not zeroed,
comment|// so zero the whole thing.
name|Arrays
operator|.
name|fill
argument_list|(
