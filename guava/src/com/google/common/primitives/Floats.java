...
begin_comment
comment|/**  * Static utility methods pertaining to {@code float} primitives, that are not already found in  * either {@link Float} or {@link Arrays}.  *  *<p>See the Guava User Guide article on<a  * href="https://github.com/google/guava/wiki/PrimitivesExplained">primitive utilities</a>.  *  * @author Kevin Bourrillion  * @since 1.0  */
end_comment
...
return|;
block|}
comment|/**    * Returns the value nearest to {@code value} which is within the closed range {@code [min..max]}.    *    *<p>If {@code value} is within the range {@code [min..max]}, {@code value} is returned    * unchanged. If {@code value} is less than {@code min}, {@code min} is returned, and if {@code    * value} is greater than {@code max}, {@code max} is returned.    *    * @param value the {@code float} value to constrain    * @param min the lower bound (inclusive) of the range to constrain {@code value} to    * @param max the upper bound (inclusive) of the range to constrain {@code value} to    * @throws IllegalArgumentException if {@code min> max}    * @since 21.0    */
annotation|@
name|Beta
DECL|method|constrainToRange (float value, float min, float max)
specifier|public
specifier|static
name|float
...
block|{
return|return
literal|"Floats.lexicographicalComparator()"
return|;
block|}
block|}
comment|/**    * Sorts the elements of {@code array} in descending order.    *    *<p>Note that this method uses the total order imposed by {@link Float#compare}, which treats    * all NaN values as equal and 0.0 as greater than -0.0.    *    * @since 23.1    */
DECL|method|sortDescending (float[] array)
specifier|public
specifier|static
name|void
name|sortDescending
...
expr_stmt|;
block|}
comment|/**    * Sorts the elements of {@code array} between {@code fromIndex} inclusive and {@code toIndex}    * exclusive in descending order.    *    *<p>Note that this method uses the total order imposed by {@link Float#compare}, which treats    * all NaN values as equal and 0.0 as greater than -0.0.    *    * @since 23.1    */
DECL|method|sortDescending (float[] array, int fromIndex, int toIndex)
specifier|public
specifier|static
name|void
name|sortDescending
...
expr_stmt|;
block|}
comment|/**    * Reverses the elements of {@code array}. This is equivalent to {@code    * Collections.reverse(Floats.asList(array))}, but is likely to be more efficient.    *    * @since 23.1    */
DECL|method|reverse (float[] array)
specifier|public
specifier|static
name|void
name|reverse
...
expr_stmt|;
block|}
comment|/**    * Reverses the elements of {@code array} between {@code fromIndex} inclusive and {@code toIndex}    * exclusive. This is equivalent to {@code    * Collections.reverse(Floats.asList(array).subList(fromIndex, toIndex))}, but is likely to be    * more efficient.    *    * @throws IndexOutOfBoundsException if {@code fromIndex< 0}, {@code toIndex> array.length}, or    *     {@code toIndex> fromIndex}    * @since 23.1    */
DECL|method|reverse (float[] array, int fromIndex, int toIndex)
specifier|public
specifier|static
name|void
name|reverse
