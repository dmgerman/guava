...
return|;
block|}
comment|/*      * Otherwise, approximate the quotient, check, and correct if necessary. Our approximation is      * guaranteed to be either exact or one less than the correct value. This follows from the fact      * that floor(floor(x)/i) == floor(x/i) for any real x and integer i != 0. The proof is not      * quite trivial.      */
name|long
name|quotient
init|=
operator|(
...
return|;
block|}
comment|/*    * We move the static constants into this class so ProGuard can inline UnsignedLongs entirely    * unless the user is actually calling a parse method.    */
DECL|class|ParseOverflowDetection
specifier|private
specifier|static
specifier|final
class|class
name|ParseOverflowDetection
block|{
DECL|method|ParseOverflowDetection ()
specifier|private
name|ParseOverflowDetection
parameter_list|()
block|{}
...
condition|)
block|{
comment|// Radix is a power of two so we can avoid division.
name|int
name|shift
init|=
name|Integer
...
condition|)
block|{
comment|// Fast path for the usual case where the radix is even.
name|quotient
operator|=
operator|(
name|x
