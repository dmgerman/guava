...
argument_list|)
expr_stmt|;
comment|// Being conservative here because the user could use resolveType() to resolve a type in an
comment|// invariant context.
return|return
name|of
argument_list|(
name|getInvariantTypeResolver
argument_list|()
operator|.
name|resolveType
argument_list|(
...
comment|// First step is to figure out StringArrayList "is-a" List<E> where<E> = String.
comment|// String is then matched against<? extends CharSequence>, the supertypeArgs[0].
if|if
condition|(
operator|!
name|of
...
block|}
block|}
comment|// We only care about the case when the supertype is a non-static inner class
comment|// in which case we need to make sure the subclass's owner type is a subtype of the
comment|// supertype's owner.
return|return
name|Modifier
operator|.
name|isStatic
...
block|}
return|return
name|canonicalizeWildcardsInType
argument_list|(
name|runtimeType
argument_list|)
operator|.
...
block|{
return|return
name|canonicalizeWildcardsInParameterizedType
argument_list|(
operator|(
name|ParameterizedType
operator|)
