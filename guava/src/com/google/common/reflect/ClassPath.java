...
name|ClassPath
block|{
comment|/**    * An item on the classpath.    *    *<p>This is a class-file or some other resource, possibly from a jar.    */
DECL|class|Entry
specifier|static
specifier|final
class|class
...
name|url
decl_stmt|;
DECL|method|Entry (File file, String resourceName, ClassLoader classLoader, URL url)
name|Entry
parameter_list|(
name|File
name|file
...
expr_stmt|;
block|}
comment|/**      * Returns the file containing this entry.  It could be a jar file, in which case the resource      * is an entry in the jar, or it could be a file, in which case the resource is the file.      */
DECL|method|file ()
specifier|public
specifier|final
name|File
...
return|;
block|}
comment|/** A {@link URL} that refers to this resource. */
DECL|method|url ()
specifier|public
specifier|final
name|URL
...
return|;
block|}
comment|/**      * Returns the name of the class represented by this entry.  Returns {@link Optional#absent()}      * if it is not a class.      */
...
block|}
block|}
comment|/**      * Returns the java resource name of the entry.      *      * @see java.lang.Object#toString()      */
...
return|;
block|}
comment|/**    * Returns an {@link Entry} for each resource on the classpath.  This method will return all items    * on the classpath, even those items that are not findable via {@link Class#getResource(String)}    * due to conflicting items that are earlier on the classpath.  Most users should use    * {@link #from(ClassLoader)}.    *    *<p>Currently only {@link URLClassLoader} and only {@code file://} urls are supported.    *    * @throws IOException if the attempt to read class path resources (jar files or directories)    *         failed.    */
DECL|method|getAllEntries (ClassLoader classloader)
specifier|static
name|ImmutableList
argument_list|<
name|Entry
...
name|className
decl_stmt|;
DECL|method|ClassInfo (String resourceName, String className, ClassLoader loader)
name|ClassInfo
parameter_list|(
name|String
name|resourceName
...
block|}
block|}
DECL|method|scanJar (final File file, ClassLoader classloader)
specifier|private
name|void
name|scanJar
parameter_list|(
