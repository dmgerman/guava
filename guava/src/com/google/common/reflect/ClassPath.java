...
name|pathUris
decl_stmt|;
DECL|method|ClassPath (ImmutableSet<URI> pathUris, ImmutableSet<ResourceInfo> resources)
specifier|private
name|ClassPath
parameter_list|(
name|ImmutableSet
...
return|;
block|}
comment|/**    * Returns the set of {@link URI} instances that comprise the class path represented by this    * instance. If this ClassPath instance was constructed using the    * {@linkplain ClassLoader#getSystemClassLoader() system class loader} then the returned URIs are    * the ones implied by the {@linkplain StandardSystemProperty#JAVA_CLASS_PATH system class path}.    * This does not include those URIs referenced via the {@code Class-Path} attribute in jar files.    *    *<p>Note that some class loaders may load classes via mechanisms that do not reference URIs, in    * which case some loadable classes may not be represented.    *    *<p>The iteration order of the result reflects the order in which the default implementation of    * {@link ClassLoader#loadClass} would attempt to find classes.    *    * @since 15.0    */
DECL|method|getPathUris ()
specifier|public
name|ImmutableSet
argument_list|<
name|URI
...
return|;
block|}
comment|/**    * Returns a representation of this class path in a format suitable for the {@code -classpath}    * flag. Path elements are separated using the {@link StandardSystemProperty#PATH_SEPARATOR}.    *    * @throws IllegalStateException if this instance references {@linkplain #pathUris URIs} that do    * not refer to files.    *    * @since 15.0    */
DECL|method|asClassPathString ()
specifier|public
name|String
name|asClassPathString
parameter_list|()
block|{
name|Set
argument_list|<
...
argument_list|(
literal|"This class path cannot be converted to string suitable for the -classpath flag."
argument_list|,
name|e
argument_list|)
throw|;
