...
return|;
block|}
comment|/**    * Construct an array of all possible chars in the slowest way possible.    */
DECL|method|slowGetChars ()
name|char
index|[]
name|slowGetChars
parameter_list|()
...
return|;
block|}
comment|/**    * This is the actual implementation of {@link #precomputed}, but we bounce calls through a method    * on {@link Platform} so that we can have different behavior in GWT.    *    *<p>If the number of matched characters is small enough, we try to build a small hash    * table to contain all of the characters. Otherwise, we record the characters in eight-kilobyte    * bit array. In many situations this produces a matcher which is faster to query    * than the original.    */
...
return|;
block|}
comment|// Otherwise, make the full lookup table.
specifier|final
name|LookupTable
name|table
init|=
