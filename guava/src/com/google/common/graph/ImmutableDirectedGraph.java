...
begin_comment
comment|/**  * Implementation of an immutable directed graph consisting of nodes  * of type N and edges of type E.  *  *<p>This class maintains the following data structures:  *<ul>  *<li>For each node: sets of incoming and outgoing edges.  *<li>For each edge: references to the source and target nodes.  *</ul>  *  *<p>Some invariants/assumptions are maintained in this implementation:  *<ul>  *<li>An edge has exactly two end-points (source node and target node), which  *     may or may not be distinct.  *<li>By default, this is not a multigraph, that is, parallel edges (multiple  *     edges directed from node1 to node2) are not allowed.  If you want a multigraph,  *     build the graph with the 'multigraph' option:  *<pre>ImmutableDirectedGraph.builder(Graphs.config().multigraph()).build();</pre>  *<li>Anti-parallel edges (same incident nodes but in opposite direction,  *     e.g. (node1, node2) and (node2, node1)) are always allowed.  *<li>Edges are not adjacent to themselves by definition. In the case of a  *     self-loop, a node can be adjacent to itself, but an edge will never be.  *</ul>  *  *<p>Most of the {@code Set}-returning accessors return an immutable set which is an internal  * data structure, hence they have a time complexity of O(1). The rest of these accessors build  * and return an immutable set that is<b>derived</b> from the internal data structures, hence  * they have the following time complexities:  *<ul>  *<li>{@code incidentEdges(node)}: O(d_node).  *<li>Methods that ask for adjacent nodes:  *<ul>  *<li>{@code adjacentNodes(node)}: O(d_node).  *<li>{@code predecessors(node)}: O(inD_node).  *<li>{@code successors(node)}: O(outD_node).  *</ul>  *<li>{@code adjacentEdges(edge)}: O(d_node1 + d_node2), where node1 and node2 are  *     {@code edge}'s incident nodes.  *<li>{@code edgesConnecting(node1, node2)}: O(min(outD_node1, inD_node2)).  *</ul>  * where d_node is the degree of node, inD_node is the in-degree of node, and outD_node is the  * out-degree of node. The set returned by these methods is<b>not</b> cached, so every time the  * user calls the method, the same set will be reconstructed again.  *  *<p>All other accessors have a time complexity of O(1), except for  * {@code degree(node)}, whose time complexity is linear in the minimum of  * the out-degree and in-degree of {@code node}. This is due to a call to {@code edgesConnecting}.  *  * @author Joshua O'Madadhain  * @param<N> Node parameter type  * @param<E> Edge parameter type  * @see Graphs  * @since 20.0  */
end_comment
...
argument_list|>
argument_list|>
name|nodeToIncidentEdgesBuilder
init|=
name|ImmutableMap
operator|.
name|builder
