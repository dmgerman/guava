...
condition|)
block|{
comment|// This is a guard for the case of the childless uncle. No checks are
comment|// performed for childlessness (even if we could check it), but since
comment|// it is the minimum sibling that is moved from "max" to "min" half
comment|// of the heap, and only if x is larger, and this is at the bottom
comment|// edge of the heap, the heap invariant is still preserved.
name|int
name|grandparentIndex
init|=
name|getParentIndex
