...
return|;
block|}
comment|/**    * The builder class used in creation of min-max priority queues. Instead of    * constructing one directly, use {@link MinMaxPriorityQueue#orderedBy},    * {@link MinMaxPriorityQueue#expectedSize} or {@link    * MinMaxPriorityQueue#maximumSize}.    *    * @param<B> the upper bound on the eventual type that can be produced by    *     this builder (for example, a {@code Builder<Number>} can produce a    *     {@code Queue<Number>} or {@code Queue<Integer>} but not a {@code    *     Queue<Object>}).    */
DECL|class|Builder
specifier|public
specifier|static
specifier|final
