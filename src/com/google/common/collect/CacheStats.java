...
DECL|method|CacheStats (long hitCount, long missCount, long createCount, long evictionCount, long totalCreateTime)
specifier|public
name|CacheStats
parameter_list|(
name|long
...
return|;
block|}
comment|/**    * Returns the number of times {@link Cache#get} has returned an uncached (newly computed) value,    * or null. Multiple concurrent calls to {@link Cache#get} on an absent value can result in    * multiple misses, all returning the results of a single computation.    */
...
return|;
block|}
comment|/**    * Returns the number of times {@link Cache#get} has successfully computed a new value. This    * differs from {@link #missCount} only in the face of concurrent calls to {@link Cache#get} on an    * absent value, in which case multiple simultaneous misses will result in a single computation.    * Thus, the returned value can never exceed the value of {@code missCount()}.    */
...
return|;
block|}
comment|/**    * Returns the total number of nanoseconds the cache has spent computing new values. This can be    * used to calculate the miss penalty. This value is increased every time {@code createCount()}    * is incremented.    */
...
return|;
block|}
comment|/**    * Returns the average time spent computing new values. This is defined as    * {@code totalCreateTime() / createCount()}.    */
