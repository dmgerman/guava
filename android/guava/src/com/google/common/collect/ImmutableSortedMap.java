...
return|;
block|}
DECL|method|fromEntries ( final Comparator<? super K> comparator, boolean sameComparator, @Nullable Entry<K, V>[] entryArray, int size)
specifier|private
specifier|static
parameter_list|<
name|K
...
expr_stmt|;
block|}
DECL|method|Builder (Comparator<? super K> comparator, int initialCapacity)
specifier|private
name|Builder
parameter_list|(
name|Comparator
...
annotation|@
name|CanIgnoreReturnValue
DECL|method|combine (ImmutableSortedMap.Builder<K, V> other)
name|Builder
argument_list|<
name|K
argument_list|,
...
index|]
decl_stmt|;
comment|// We might, somehow, be able to reorder values in-place.  But it doesn't seem like
comment|// there's a way around creating the separate sortedKeys array, and if we're allocating
comment|// one array of size n, we might as well allocate two -- to say nothing of the allocation
comment|// done in Arrays.sort.
for|for
control|(
name|int
name|i
...
argument_list|(
literal|"keys required to be distinct but compared as equal: "
operator|+
name|sortedKeys
index|[
name|i
...
name|comparator
decl_stmt|;
DECL|method|SerializedForm (ImmutableSortedMap<K, V> sortedMap)
name|SerializedForm
parameter_list|(
name|ImmutableSortedMap
argument_list|<
