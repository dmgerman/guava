...
literal|1
decl_stmt|;
comment|// Max size is halved due to indexing into double-sized alternatingKeysAndValues
...
condition|)
block|{
comment|/*        * Use 8 bits per entry. The value is unsigned to allow use up to a size of 2^8.        *        * The absent indicator of -1 signed becomes 2^8 - 1 unsigned, which reduces the actual max        * size to 2^8 - 1. However, due to a load factor< 1 the limit is never approached.        */
name|byte
index|[]
name|hashTable
init|=
...
condition|)
block|{
comment|// -1 signed becomes 255 unsigned
name|hashTable
index|[
name|h
index|]
...
condition|)
block|{
comment|/*        * Use 16 bits per entry. The value is unsigned to allow use up to a size of 2^16.        *        * The absent indicator of -1 signed becomes 2^16 - 1 unsigned, which reduces the actual max        * size to 2^16 - 1. However, due to a load factor< 1 the limit is never approached.        */
name|short
index|[]
name|hashTable
init|=
...
condition|)
block|{
comment|// -1 signed becomes 65_535 unsigned
name|hashTable
index|[
name|h
index|]
