...
expr_stmt|;
block|}
DECL|method|WildcardCapturer (AtomicInteger id)
specifier|private
name|WildcardCapturer
parameter_list|(
name|AtomicInteger
...
throw|;
block|}
DECL|method|captureAsTypeVariable (Type[] upperBounds)
name|TypeVariable
argument_list|<
name|?
argument_list|>
...
comment|// adds recursive isSubtypeOf() call and feels complicated.
comment|// There is no contract one way or another as long as isSubtypeOf() works as expected.
name|combined
operator|.
name|addAll
argument_list|(
...
condition|)
block|{
comment|// Object is implicit and only useful if it's the only bound.
name|combined
operator|.
name|remove
argument_list|(
...
return|;
block|}
DECL|method|notForTypeVariable ()
specifier|private
name|WildcardCapturer
name|notForTypeVariable
parameter_list|()
