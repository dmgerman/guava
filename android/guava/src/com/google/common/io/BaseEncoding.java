...
name|DecodingException
function_decl|;
DECL|method|trimTrailingPadding (CharSequence chars)
name|CharSequence
name|trimTrailingPadding
parameter_list|(
name|CharSequence
...
name|DecodingException
argument_list|(
literal|"Unrecognized character: 0x"
operator|+
name|Integer
operator|.
name|toHexString
...
name|IOException
block|{
comment|// Overriding this to work around the fact that InputStream's default implementation of
comment|// this method will silently swallow exceptions thrown by the single-byte read() method
comment|// (other than on the first call to it), which in this case can cause invalid encoded
comment|// strings to not throw an exception.
...
argument_list|)
argument_list|,
literal|"Separator (%s) cannot contain alphabet characters"
argument_list|,
name|separator
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"Separator (%s) cannot contain padding character"
argument_list|,
name|separator
argument_list|)
expr_stmt|;
